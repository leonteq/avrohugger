...
class SpecificGeneratorToolSpec extends mutable.Specification {
  ...
  "match the expected single protocol file" in {
    doCompile(Array[String]("protocol", Directory.TEST_INPUT_DIR + "mail.avpr", Directory.TEST_OUTPUT_SPECIFIC_BASE_DIR))
  ...
  "match the expected single datafile file" in {
    doCompile(Array[String]("datafile", Directory.TEST_INPUT_DIR + "twitter.avro", Directory.TEST_OUTPUT_SPECIFIC_BASE_DIR))
  ...
  "match the expected single schema file" in {
    doCompile(Array[String]("schema", Directory.TEST_INPUT_DIR + "mascot.avsc", Directory.TEST_OUTPUT_SPECIFIC_BASE_DIR))
  ...
  "match the expected file and directory" in {
    doCompile(
      Array[String]("schema", Directory.TEST_INPUT_DIR + "/mascot.avsc", Directory.TEST_INPUT_DIR, Directory.TEST_OUTPUT_SPECIFIC_BASE_DIR)
    )
