...
class MainSpec extends mutable.Specification {
  ...
  "successful runs yield zero exit code" in {
    ...
    val exitCode = r.run(Array("generate", "schema", Directory.TEST_INPUT_DIR + "/handle.avsc", Directory.TEST_OUTPUT_BASE_DIR))
    exitCode === 0
  }
  ...
  "not print anything to stderr" in {
    val errBytes: ByteArrayOutputStream = new ByteArrayOutputStream()
    val r:        Runner                = new Runner(null, null, new PrintStream(errBytes))
    r.run(Array("generate", "schema", Directory.TEST_INPUT_DIR + "/handle.avsc", Directory.TEST_OUTPUT_BASE_DIR))
    errBytes.toString() === ""
  }
  ...
  "invalid tool names yield non-zero exit code" in {
    val errBytes: ByteArrayOutputStream = new ByteArrayOutputStream()
    val r:        Runner                = new Runner(null, null, new PrintStream(errBytes))
    val exitCode = r.run(Array("no-such-tool", "schema", Directory.TEST_INPUT_DIR + "/handle.avsc", Directory.TEST_OUTPUT_BASE_DIR))
    exitCode !== 0
  }
  ...
  "print something to stderr" in {
    val errBytes: ByteArrayOutputStream = new ByteArrayOutputStream()
    val r:        Runner                = new Runner(null, null, new PrintStream(errBytes))
    r.run(Array("no-such-tool", "schema", Directory.TEST_INPUT_DIR + "/handle.avsc", Directory.TEST_OUTPUT_BASE_DIR))
  ...
  "invalid input yield non-zero exit code" in {
    val errBytes: ByteArrayOutputStream = new ByteArrayOutputStream()
    val r:        Runner                = new Runner(null, null, new PrintStream(errBytes))
    val exitCode = r.run(Array("generate", "schema", Directory.TEST_INPUT_DIR + "/invalid.avsc", Directory.TEST_OUTPUT_BASE_DIR))
    exitCode !== 0
  }
  ...
  "print something to stderr" in {
    val errBytes: ByteArrayOutputStream = new ByteArrayOutputStream()
    val r:        Runner                = new Runner(null, null, new PrintStream(errBytes))
    r.run(Array("generate", "schema", Directory.TEST_INPUT_DIR + "/invalid.avsc", Directory.TEST_OUTPUT_BASE_DIR))
