...
/** A Tool for generating Scala case classes from schemas Adapted from
  * https://github.com/apache/avro/blob/branch-1.7/lang/java/tools/src/main/java/org/apache/avro/tool/SpecificCompilerTool.java
  */
class GeneratorTool(
  ...
  /** For a List of files or directories, returns a File[] containing each file passed as well as each file with a matching extension found
    * in the directory.
    *
    * @param inputs
    *   List of File objects that are files or directories
    * @param filter
    *   File extension filter to match on when fetching files from a directory
    * @return
    *   Unique array of files
    */
