...
object CustomDefaultParamMatcher {
  ...
  def checkCustomDecimalType(
    decimalType:  AvroScalaDecimalType,
    schema:       Schema,
    default:      => Tree,
    decimalValue: => Option[String] = None
  ) = {
    val decimalValueRef = REF("scala.math.BigDecimal") APPLY decimalValue.map(LIT(_)).getOrElse(LIT(0))
    LogicalType.foldLogicalTypes[Tree](schema = schema, default = default) { case Decimal(precision, scale) =>
      decimalType match {
  ...
  def checkCustomTimestampMicrosType(timeMillisType: AvroScalaTimestampType): Tree =
    timeMillisType match {
      case JavaTimeZonedDateTime =>
        REF("java.time.ZonedDateTime.of").APPLY(REF("java.time.LocalDateTime") DOT "MIN", REF("java.time.ZoneId") DOT "of" APPLY LIT("UTC"))
      case UnderlyingPrimitive => LIT(0L)
    }
