  ...
  def getSchemaOrProtocols(
    ...
    val res = infile.getName.split("\\.").last match {
      ...
      case "avdl" =>
        ...
        /** IDLs may refer to types imported from another file. When converted to protocols, the imported types that share the IDL's
          * namespace cannot be distinguished from types defined within the IDL, yet should not be generated as subtypes of the IDL's ADT
          * and should instead be generated in its own namespace. So, strip the protocol of all imported types and generate them separately.
          */
        ...
        def stripImports(protocol: Protocol, importedSchemaOrProtocols: Set[Either[Schema, Protocol]]) = {
          ...
          val localTypes =
            imported.foldLeft(types)((remaining, imported) => remaining.filterNot(remainingType => remainingType == imported))
