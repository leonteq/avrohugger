...
object SpecificRecord extends SourceFormat {
  ...
  def asCompilationUnits(
    ...
    val namespace =
      CustomNamespaceMatcher.checkCustomNamespace(ns, typeMatcher, maybeDefaultNamespace = ns)
    ...
    def protocolToRPC(protocol: Protocol): List[CompilationUnit] = {
      ...
      val maybePath     = getFilePath(namespace, Right(protocol), maybeOutDir, typeMatcher)
      ...
      val javaCompUnits    = localEnums.map(schema => getJavaEnumCompilationUnit(classStore, namespace, schema, maybeOutDir, typeMatcher))
    ...
    schemaOrProtocol match {
          ...
          case ENUM =>
            enumType match {
              case JavaEnum =>
                ...
                val javaCompilationUnit = getJavaEnumCompilationUnit(classStore, namespace, schema, maybeOutDir, typeMatcher)
      ...
      case Right(protocol) =>
        ...
        if (messages.isEmpty) {
          ...
          val javaCompilationUnits =
            localEnums.map(schema => getJavaEnumCompilationUnit(classStore, namespace, schema, maybeOutDir, typeMatcher))
          val scalaCompilationUnit = getScalaCompilationUnit(
            classStore,
            namespace,
            Right(protocol),
