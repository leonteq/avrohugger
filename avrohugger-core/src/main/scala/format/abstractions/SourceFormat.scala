...
/** Parent to all ouput formats
  *
  * _ABSTRACT MEMBERS_: to be implemented by a subclass asCompilationUnits compile defaultTypes getName javaTreehugger scalaTreehugger
  * toolName toolShortDescription
  *
  * _CONCRETE MEMBERS_: implementations to be inherited by a subclass fileExt getFilePath getLocalSubtypes getJavaEnumCompilationUnit
  * getScalaCompilationUnit isEnum registerTypes renameEnum RESERVED_WORDS writeToFile
  */
trait SourceFormat {
  ...
  def getJavaEnumCompilationUnit(
    ...
    val codeString = javaTreehugger.asJavaCodeString(classStore, namespace, schema)
