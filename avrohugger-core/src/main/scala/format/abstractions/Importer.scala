...
/** Parent to all ouput formats' importers
  ...
  *
  * _CONCRETE MEMBERS_: implementations to be inherited by a subclass getEnumSchemas getFieldSchemas getUserDefinedImports getRecordSchemas
  * getTopLevelSchemas isEnum isRecord
  */
trait Importer {
  ...
  def getEnumSchemas(topLevelSchemas: Set[Schema], alreadyImported: Set[Schema] = Set.empty[Schema]): Set[Schema] = {
  ...
  def getUserDefinedImports(recordSchemas: Set[Schema], namespace: Option[String], typeMatcher: TypeMatcher): Set[Import] = {

    def checkNamespace(schema: Schema): Option[String] = {
      val maybeReferredNamespace =
        DependencyInspector.getReferredNamespace(schema)
      CustomNamespaceMatcher.checkCustomNamespace(maybeReferredNamespace, typeMatcher, maybeDefaultNamespace = maybeReferredNamespace)
    }

    def asImportDef(packageName: String, fields: Set[Schema]): Import = {
      val maybeUpdatedPackageName =
        CustomNamespaceMatcher.checkCustomNamespace(Some(packageName), typeMatcher, maybeDefaultNamespace = Some(packageName))
    ...
    recordSchemas
      ...
      .map { case (packageName: String, fields: Set[Schema]) =>
        asImportDef(packageName, fields)
      }
      .toSet
  }

  // gets record schemas which may be dependencies
  def getRecordSchemas(topLevelSchemas: Set[Schema], alreadyImported: Set[Schema] = Set.empty[Schema]): Set[Schema] = {
    def nextSchemas(s: Schema, us: Set[Schema]) = getRecordSchemas(Set(s), us)
  ...
  def getTopLevelSchemas(schemaOrProtocol: Either[Schema, Protocol], schemaStore: SchemaStore, typeMatcher: TypeMatcher): Set[Schema] =
    schemaOrProtocol match {
      ...
      case Right(protocol) =>
        protocol
          ...
          .asScala
          .toList
          .flatMap(schema => NestedSchemaExtractor.getNestedSchemas(schema, schemaStore, typeMatcher) + schema)
          .toSet
    }
