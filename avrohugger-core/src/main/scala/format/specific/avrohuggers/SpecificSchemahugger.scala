  ...
  def toTrees(
    ...
    schema.getType match {
      case RECORD =>
        ...
        val companionDef = SpecificObjectTree.toCaseCompanionDef(schema, maybeFlags, schemaStore, typeMatcher)
        List(caseClassDef, companionDef)
      case FIXED =>
        val caseClassDef =
          SpecificCaseClassTree.toFixedDef(schema, namespace, maybeFlags, schemaStore, typeMatcher, classStore, targetScalaPartialVersion)
        val companionDef = SpecificObjectTree.toCaseCompanionDef(schema, maybeFlags, schemaStore, typeMatcher)
        List(caseClassDef, companionDef)
