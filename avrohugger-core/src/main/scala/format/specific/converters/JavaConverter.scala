...
object JavaConverter {
  ...
  def convertToJava(
    ...
    case Schema.Type.UNION =>
      ...
      if (hasNull) {
        val conversionCases = List(
          CASE(SOME(ID("x"))) ==> {
            convertToJava(typeParamSchemas.head, schemaAccessor, true, REF("x"), classSymbol, typeMatcher, targetScalaPartialVersion)
          },
          CASE(NONE) ==> {
            NULL
          }
    ...
    case Schema.Type.ARRAY =>
      val applyParam =
        BLOCK(
          tree MAP (LAMBDA(PARAM("x")) ==> BLOCK(
            convertToJava(
              schema.getElementType,
              if (isUnionMember)
                arrayAccessor(
                  unionAccessor(schemaAccessor, schema.getFullName, ScalaConverter.asScalaBufferConverter(targetScalaPartialVersion))
                )
        ...
        case _: LogicalTypes.TimestampMicros =>
          (typeMatcher.avroScalaTypes.timestampMicros match {
            case JavaTimeZonedDateTime =>
              BLOCK(tree.DOT("toEpochSecond").INFIX("*", LIT(1000000L)).INFIX("+", tree.DOT("getNano").INFIX("/", LIT(1000L))))
            case UnderlyingPrimitive => tree
          ...
          (typeMatcher.avroScalaTypes.localTimestampMillis match {
            case JavaTimeLocalDateTime =>
              BLOCK(
                tree
                  .DOT("toEpochSecond")
                  .APPLY(RootClass.newClass("java.time.ZoneOffset").DOT("UTC"))
                  .INFIX("*", LIT(1000L))
                  .INFIX("+", tree.DOT("getNano").INFIX("/", LIT(1000000L)))
              )
            case UnderlyingPrimitive => tree
            ...
            case JavaTimeLocalDateTime =>
              BLOCK(
                tree
                  ...
                  .APPLY(RootClass.newClass("java.time.ZoneOffset").DOT("UTC"))
                  .INFIX("*", LIT(1000000L))
                  .INFIX("+", tree.DOT("getNano").INFIX("/", LIT(1000L)))
              )
            case UnderlyingPrimitive => tree
    ...
    case Schema.Type.INT =>
      schema.getLogicalType match {
        ...
        case timeMillis: LogicalTypes.TimeMillis =>
    ...
    case Schema.Type.ENUM =>
      typeMatcher.avroScalaTypes.`enum` match {
        case EnumAsScalaString =>
          ...
          val schemaTree: Tree =
            if (isUnionMember)
              defaultTree.DOT("getTypes").DOT("get").APPLY(defaultTree.DOT("getIndexNamed").APPLY(LIT(schema.getFullName())))
            else defaultTree
          NEW(REF("org.apache.avro.generic.GenericData.EnumSymbol").APPLY(schemaTree, tree))
