...
object ScalaConverter {
  ...
  def checkCustomArrayType(arrayType: AvroScalaArrayType, elementType: Type, asScalaTree: Tree): Tree = {
    ...
    schema.getType match {
      case Schema.Type.ARRAY =>
        ...
        val elementConversion = convertFromJava(
          ...
          elementSchema,
          if (isUnionMember)
            arrayAccessor(unionAccessor(schemaAccessor, schema.getFullName, asScalaBufferConverter(targetScalaPartialVersion)))
          else arrayAccessor(schemaAccessor),
          false,
          REF("x"),
          typeMatcher,
          classSymbol,
        ...
        val asScalaTree: Tree =
          REF(asScalaIteratorConverter(targetScalaPartialVersion))
            .APPLY(applyParam)
            .DOT("asScala")
            .DOT("map")
            .APPLY(LAMBDA(PARAM("x")) ==> BLOCK(elementConversion))
        val arrayType       = typeMatcher.avroScalaTypes.array
        val resultExpr      = BLOCK(checkCustomArrayType(arrayType, elementType, asScalaTree))
      ...
      case Schema.Type.STRING =>
        LogicalType.foldLogicalTypes(schema = schema, default = tree TOSTRING) { case UUID =>
                ...
                LAMBDA(PARAM("kvp")) ==> BLOCK(
                  ...
                  PAREN(
                    ...
                    convertFromJava(
                      ...
                      schema.getValueType,
                      if (isUnionMember)
                        mapAccessor(unionAccessor(schemaAccessor, schema.getFullName, asScalaBufferConverter(targetScalaPartialVersion)))
                      else mapAccessor(schemaAccessor),
                      false,
                      REF("value"),
                      typeMatcher,
                      classSymbol,
      ...
      case Schema.Type.BYTES =>
        ...
        val resultExpr = schema.getLogicalType match {
          case decimal: LogicalTypes.Decimal =>
            ...
            Block(
              VAL("schema") := {
                if (isUnionMember) unionAccessor(schemaAccessor, schema.getFullName, asScalaBufferConverter(targetScalaPartialVersion))
                else schemaAccessor
              },
              VAL("decimalType") := REF("schema").DOT("getLogicalType").APPLY().AS(Decimal),
              REF("BigDecimal").APPLY(
                classSymbol.DOT("decimalConversion").DOT("fromBytes").APPLY(REF("buffer"), REF("schema"), REF("decimalType"))
              )
            )
          case _ =>
            Block(
                ...
                case JavaTimeZonedDateTime =>
                  ...
                  val resultExpr = BLOCK(
                    ZonedDateTime
                      .DOT("of")
                      .APPLY(
                        LocalDateTime DOT "ofEpochSecond" APPLY (
