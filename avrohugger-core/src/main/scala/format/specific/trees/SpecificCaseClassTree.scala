...
object SpecificCaseClassTree {
  ...
  def toCaseClassDef(
    ...
    val params: List[ValDef] = avroFields.map { f =>
      ...
      val defaultValue = DefaultValueMatcher.getDefaultValue(classStore, namespace, f, typeMatcher, fieldName == fieldType.safeToString)
    ...
    val defGetSchema =
      namespace.fold(GetSchemaGenerator(classSymbol).toDef)(ns => GetSchemaGenerator(RootClass.newClass(s"$ns.${classSymbol}")).toDef)
    ...
    val caseClassTree =
      ...
      if (!avroFields.isEmpty) caseClassDef := BLOCK(defThis, defGet, defPut, defGetSchema)
      else caseClassDef := BLOCK(defGet, defPut, defGetSchema)

    val treeWithScalaDoc = ScalaDocGenerator.docToScalaDoc(Left(schema), caseClassTree)
