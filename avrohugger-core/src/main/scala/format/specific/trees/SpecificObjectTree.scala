...
object SpecificObjectTree {
  ...
  def schemaContainsDecimal(schema: Schema, schemaStore: SchemaStore, typeMatcher: TypeMatcher): Boolean = {
  ...
  def toCaseCompanionDef(schema: Schema, maybeFlags: Option[List[Long]], schemaStore: SchemaStore, typeMatcher: TypeMatcher) = {
    ...
    val schemaDef = VAL(REF("SCHEMA$")) := {
      NEW(ParserClass) APPLY Nil DOT "parse" APPLY (schema.toString match {
        case schemaString if schemaString.getBytes(StandardCharsets.UTF_8).length > 65535 =>
          LIST(schemaString.split("},\\{\"").map(LIT)) DOT "mkString" APPLY (LIT("},{\""))
        case schemaString => LIT(schemaString)
      })
    }
    val externalReader = VAL("READER$") := NEW("org.apache.avro.specific.SpecificDatumReader")
      .APPLYTYPE(TYPE_REF(schema.getName))
      .APPLY(REF(s"${schema.getFullName()}.SCHEMA$$"))
    val externalWriter = VAL("WRITER$") := NEW("org.apache.avro.specific.SpecificDatumWriter")
      .APPLYTYPE(TYPE_REF(schema.getName))
      .APPLY(REF(s"${schema.getFullName()}.SCHEMA$$"))
    ...
    val defCtor = {
      ...
      DEF("apply", TYPE_REF(schema.getName()))
        .withParams(PARAM("data", CustomTypeMatcher.checkCustomDecimalType(typeMatcher.avroScalaTypes.decimal, schema))) := Block(
        ...
        REF("fixed")
          .DOT("bytes")
          .APPLY(REF("decimalConversion").DOT("toBytes").APPLY(REF("bigDecimal"), REF("schema"), REF("decimalType")).DOT("array").APPLY()),
        REF("fixed")
      )
  ...
  def toTraitCompanionDef(protocol: Protocol) = {
    ...
    // companion object definition
    OBJECTDEF(protocol.getName) := BLOCK(VAL("PROTOCOL", ProtocolClass).withFlags(Flags.FINAL) := {
