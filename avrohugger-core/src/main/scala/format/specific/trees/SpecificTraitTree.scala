...
object SpecificTraitTree {
  ...
  def toADTRootDef(protocol: Protocol) = {
    ...
    val treeWithScalaDoc = ScalaDocGenerator.docToScalaDoc(Right(protocol), adtRootTree)

    treeWithScalaDoc
  }

  def toRPCTraitDef(classStore: ClassStore, namespace: Option[String], protocol: Protocol, typeMatcher: TypeMatcher) = {
    ...
    val callbackTraitTree =
      TRAITDEF("Callback").withAnnots(suppressWarn).withParents(name) := BLOCK(protocolVal +: callbackDefs)

    // Tree
    val traitTree =
      TRAITDEF(name).withAnnots(suppressWarn, avroGenerated) := BLOCK(messageDefs :+ callbackTraitTree)

    // Return
    val treeWithScalaDoc = ScalaDocGenerator.docToScalaDoc(Right(protocol), traitTree)
    treeWithScalaDoc
