...
object StandardImporter extends Importer {
  ...
  private[this] def getShapelessImports(topLevelRecordSchemas: Set[Schema], typeMatcher: TypeMatcher): Set[Import] = {
    ...
    def determineShapelessCoproductImports(
      ...
      potentialRecursives: List[Schema]
    ): List[String] = schema.getType match {
      case Schema.Type.UNION =>
        coproductImportsForUnionType(field, schema, typeMatcher) ++
          schema.getTypes().asScala.toList.flatMap(s => determineShapelessCoproductImports(field, s, typeMatcher, potentialRecursives))
    ...
    def determineShapelessTagImport(schema: Schema, typeMatcher: TypeMatcher, potentialRecursives: List[Schema]): List[String] =
      schema.getType match {
        case Schema.Type.UNION =>
          schema.getTypes().asScala.toList.flatMap(s => determineShapelessTagImport(s, typeMatcher, potentialRecursives))
    ...
    def coproductImportsForUnionType(field: Schema.Field, unionSchema: Schema, typeMatcher: TypeMatcher): List[String] = {
