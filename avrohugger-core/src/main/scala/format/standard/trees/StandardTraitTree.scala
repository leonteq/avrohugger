...
object StandardTraitTree {
  ...
  def toCaseObjectEnumDef(schema: Schema, maybeBaseTrait: Option[String]): List[Tree] = {
    ...
    val adtSubTypes: List[Tree] = schema.getEnumSymbols.asScala
      ...
      .map(enumSymbolString => CASEOBJECTDEF(enumSymbolString).withParents(schema.getName): Tree)
      .toList
    val objectTree = OBJECTDEF(schema.getName) := Block(adtSubTypes: _*)
    val adtRootTreeWithScalaDoc: Tree = ScalaDocGenerator.docToScalaDoc(Left(schema), adtRootTree)
