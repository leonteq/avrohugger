...
object StandardCaseClassTree {
  ...
  def toCaseClassDef(
    ...
    val params: List[ValDef] = avroFields.map { f =>
      ...
      val defaultValue = DefaultValueMatcher.getDefaultValue(classStore, namespace, f, typeMatcher, fieldName == f.schema.getName())
    ...
    val treeWithScalaDoc = ScalaDocGenerator.docToScalaDoc(Left(schema), classTree)

    treeWithScalaDoc
  }

  def toFixedClassDef(classStore: ClassStore, namespace: Option[String], schema: Schema, typeMatcher: TypeMatcher) = {
