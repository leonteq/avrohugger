...
object StandardObjectTree {
  ...
  def toCaseCompanionDef(schema: Schema, maybeFlags: Option[List[Long]]) = {
    ...
    objectDef := BLOCK(VAL(REF("SCHEMA$")) := {
      (NEW(ParserClass)) APPLY Nil DOT "parse" APPLY (LIT(schema.toString))
    })
  }

  def toScalaEnumDef(classStore: ClassStore, schema: Schema, maybeBaseTrait: Option[String], maybeFlags: Option[List[Long]]) = {
    ...
    val objectTree =
      objectDef := BLOCK(TYPEVAR(schema.getName) := REF("Value"), VAL(schema.getEnumSymbols.asScala.mkString(", ")) := REF("Value"))

    val treeWithScalaDoc = ScalaDocGenerator.docToScalaDoc(Left(schema), objectTree)
