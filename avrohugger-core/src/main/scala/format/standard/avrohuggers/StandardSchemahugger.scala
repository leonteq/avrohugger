...
object StandardSchemahugger extends Schemahugger {
  ...
  def toTrees(
    ...
    schema.getType match {
      case RECORD =>
        val classDef =
          StandardCaseClassTree.toCaseClassDef(classStore, namespace, schema, typeMatcher, maybeBaseTrait, maybeFlags, restrictedFields)
        val companionDef = StandardObjectTree.toCaseCompanionDef(schema, maybeFlags)
      ...
      case ENUM =>
        typeMatcher.avroScalaTypes.`enum` match {
          ...
          case ScalaEnumeration =>
            val objectDef = StandardObjectTree.toScalaEnumDef(classStore, schema, maybeBaseTrait, maybeFlags)
            List(objectDef)
          case EnumAsScalaString => List.empty
        }
      case FIXED =>
        val classDef     = StandardCaseClassTree.toFixedClassDef(classStore, namespace, schema, typeMatcher)
        val companionDef = StandardObjectTree.toCaseCompanionDef(schema, None)
