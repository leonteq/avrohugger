...
//    correctly generate logical types from IDL $e26
  ...
  def e19 = {
    ...
    val sources = classes.map(className =>
      util.Util.readFile(s"avrohugger-core/src/test/expected/specific/com/example/avrohugger/unions_with_coproduct_avsc/$className.scala")
    )
  ...
  def e20: Result = Result.forall(Seq(OptionalShapelessCoproduct, OptionShapelessCoproduct, OptionEitherShapelessCoproduct)) { unionType =>
    ...
    Result.forall(
      ...
      val generatedFile =
        s"target/generated-sources/specific/$unionType/com/example/avrohugger/unions_with_coproduct_avsc2/$className.scala"
      val expectationFile =
        s"avrohugger-core/src/test/expected/specific/$unionType/com/example/avrohugger/unions_with_coproduct_avsc2/$className.scala"
  ...
  def e22 = {
    ...
    } yield List(orderSource, reportSource, userSource)
  ...
  def e27 = {
    ...
    val avroScalaCustomTypes =
      SpecificRecord.defaultTypes.copy(date = JavaSqlDate, timestampMillis = JavaSqlTimestamp, timeMillis = JavaSqlTime)
  ...
  def e29 = {
    val infile = new java.io.File("avrohugger-core/src/test/avro/mail.avpr")
    val gen    = new Generator(SpecificRecord, avroScalaCustomNamespace = Map("example.*" -> "example.protocol"))
    val outDir = gen.defaultOutputDir + "/specific"
    gen.fileToFile(infile, outDir)
  ...
  def e31 = {
    ...
    val avroScalaCustomTypes = SpecificRecord.defaultTypes.copy(
      date                 = UnderlyingPrimitive,
      timestampMillis      = UnderlyingPrimitive,
      timestampMicros      = UnderlyingPrimitive,
      localTimestampMicros = UnderlyingPrimitive,
      localTimestampMillis = UnderlyingPrimitive,
      timeMillis           = UnderlyingPrimitive,
      timeMicros           = UnderlyingPrimitive
    )
    val gen    = new Generator(SpecificRecord, avroScalaCustomTypes = Some(avroScalaCustomTypes))
    val outDir = gen.defaultOutputDir + "/specific/"
    gen.fileToFile(infile, outDir)
