...
class SpecificStringToFileSpec extends Specification {
  ...
  def e14 = {
    ...
    val gen         = new Generator(SpecificRecord)
    val outDir      = gen.defaultOutputDir + "/specific/"
    gen.stringToFile(inputString, outDir) must throwA(
      new java.lang.RuntimeException("Imports not supported in String IDLs, only avdl files.")
    )
  }
  ...
  def e23 = {
    ...
    val schema = {
      ...
      Stream
        .from(1)
        .scanLeft(s"""{"type":"record","name":"massive._0","fields":[$fields]}""") { case (field, i) =>
